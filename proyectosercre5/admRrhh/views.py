...
def zonGuardar(resquest):
    if resquest.method == 'POST':
        ...
        zonas = Zona.objects.create(nombre_zon=nombre.capitalize())
        messages.success(resquest, " SE REGISTRO CORRECTAMENTE ")
...
def zonActualizar(resquest, id):
    if resquest.method == 'POST':
        ...
        zona.nombre_zon = nuevo_nombre.capitalize()
        zona.save()
        messages.success(resquest, "Se actualizó correctamente el área.")
        return redirect('zonLista')
    else:
...
def asigEliminarD(resquest, id):
    proyecto_id = asignacion.proyecto_asig_id
...
def reproyLista(resquest):
    ...
    'proyectos_sin_asignacion': proyectos_sin_asignacion, 'asignaciones': asignaciones, 'asignaciones_destacadas': asignaciones_destacadas, 
    })
